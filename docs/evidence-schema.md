...
| gym.json (gym score output) | `{<finding-id>:{artifact, candidate, prevented:{n,of}, preserved:{n,of}, unscorable, reason, errors, skipped_budget, tokens_est, per_case:[{case, side, verdict, prompt_hash, error?}]}}` — one `scripts/gym.py score` result per qualifying finding, merged by the runner; report.py renders BOTH sides on the finding. Judged evidence for the human decision, never an auto-gate |
