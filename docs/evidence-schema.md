...
| artifacts.json | `{artifacts:[{id, kind:"skill" or "agent" or "analyst", source, path, activation_count, symlinked, body, self_corpus?, front?}]}` — the ≤10 most-activated user-owned skills + any-source agents, bodies truncated to 8000 chars; a `kind:"analyst"` entry is one of the plugin's own analyst instruction files, appended only once its self-corpus holds at least `gym.min_cases_per_side` failure cases, and it carries `self_corpus:{failure,working}` (the counts that gate it) — see Self-application below; `symlinked` is true when the file or a parent dir (≤3 levels) is a symlink. `front` is present only for an artifact with at least `gym.front.min_failure_cases` recorded failure cases AND two or more scored versions in its archive: `{failure_cases, trade_off, members:[{variant, parent, run_id, prevented:{n,of,rate}, preserved:{n,of,rate}, ops:[{op,anchor,text}], title, expected_improvement}]}` — the Pareto front over both score sides, at most `gym.front.max_members` versions, extremes kept |
...
| findings.json | `{findings:[rec + id + evidence_hash + analyst + delta_tokens + deep_localize? + enforcement? + self_edit?], dropped:{invalid,citations,citation_checked,guard,suppressed,citation_detail:[{analyst,title,failed_refs}],self_capped:[{analyst,title}]}}` — `analyst` is the name of the analyst that proposed the finding, carried into the ledger so a later run can attribute the outcome back to it; `self_edit` is `{analyst,failure_cases,working_cases}` on a finding that edits an analyst instruction file; `citation_checked` is the denominator for the citation-resolution rate; `self_capped` lists self-edits dropped by the one-per-analyst-per-run cap; `citation_detail` drives the runner's single citation-retry round; `deep_localize` (optional) is synth.py's attachment of any matching localize.json rows, advisory supporting evidence, never a finding category of its own; `enforcement` (optional) is the runtime check a durable correction proposes — see below |
| verify.json (verify step output) | `{rows:[per-applied-rec metric verdicts], enforcement:[{id, title, category, predicted, adopted_at, baseline_run, observed_run, baseline:{count,total,share}, observed:{count,total,share}, rel_change, verdict, note}]}, self_edits:[{id, title, artifact, applied_at, before_run, after_run, rates:{citation_rate?:{before,after,rel_change}, verified_rate?:{…}}, verdict, note}]}` — `rows` is the metric verification report.py and dashboard.py render as "Applied changes: outcomes"; `enforcement` is the predicted-vs-observed scoring of every adopted enforcement proposal, recomputed every run and never written back as a ledger status; `self_edits` is the self-application guard — each applied analyst edit re-scored on the run that followed it, `regressed` meaning one of the two pipeline rates fell past `verify.min_rel_change` and the edit is flagged for rollback |
...
| gym/registry.json + gym/corpus/\*.json (state, not evidence) | the eval gym's derived artifact registry and per-artifact case corpus, written by `scripts/gym.py update` from the files above; registry is `{last_run, artifacts:{<id>:{id, kind, name, path, source, activation_key, self_signal?, first_seen_run, last_seen_run, absent_runs, retired}}}`, each corpus file is `{artifact, failure:[case], working:[case]}` with case `{id, harness, session, project, ts, first_seen_run, user_text, assistant_text}` — mode 0600, never committed. An `analyst:<name>` entry carries `self_signal: true` and no `activation_key`; its cases add `origin:"self"` and `signal` (`rejected`/`regressed`/`verified`/`citations`/`gym-score`) |
| gym/archive/\<artifact\>/\<variant\>.json (state, not evidence) | one file per candidate version the gym scored, written by `scripts/gym.py gate`: `{id, artifact, parent, seq, run_id, finding_id, created_at, path, base_sha, text_sha, text, ops, chain_ops, scores:{prevented,preserved,unscorable,reason}, provenance:{title, evidence_refs, expected_improvement}}` — mode 0600 under 0700 dirs, never committed. `parent` is the variant these edits were written against (null = written against the live artifact) and is derived from `base_sha` unless the finding declared one. `ops` is the delta from that parent; `chain_ops` is the flattened set that applies to the live artifact. A `file_ops` payload may declare `parent_variant` (an archived variant id) plus `expected_improvement`; the gate flattens such a payload onto its parent's chain and marks it `ops_composed` |
...
The plugin's own analyst instruction files (`agents/*.md`) register as gym artifacts
under the ids `analyst:transcript-miner`, `analyst:config-auditor`, `analyst:evolver`
and `analyst:sample-labeler`. They are registered on every run whatever the inventory
says, since they are not part of any harness's config surface, and they never retire.

Their cases come from the pipeline's own records rather than from session transcripts
(`scripts/selfsignal.py`), attributed by the `analyst` field synth.py stamps on each
finding:

| case | source |
|---|---|
| failure | ledger status `rejected` or `regressed`; a `citation_detail` drop in any past run's findings.json; a gym score that prevented nothing or broke a working case |
| working | ledger status `verified`; a gym score that prevented at least one failure case while preserving every working case |

A self-edit is a `file_ops` action whose path is one of those files exactly (symlinks
resolved — no directory is opened up, and no other file in the plugin is reachable).
It is permanently tier B (`schema.validate_rec`), capped at one per analyst per run
(`synth.synthesize`), and re-scored on the next run by `selfsignal.self_edit_rows`
against two pipeline rates: `citation_rate` (share of recommendations whose citations
all resolved, from findings.json's `citation_checked`) and `verified_rate` (share of
terminal verify verdicts that came out verified). A fall past `verify.min_rel_change`
in either is flagged for rollback in verify.json and the report. Nothing auto-reverts.
